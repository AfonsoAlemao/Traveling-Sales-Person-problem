...
Path *copy_path(Path *initial_path_aux, int n_cities) {
    if (n_cities <= 0) return NULL;
    Path *new_path;
	new_path = NULL; 
    ...
    new_path->Tour[0] = initial_path_aux->Tour[0];
    
	for (int i = 1; i < n_cities + 1; i++) {
		new_path->Tour[i] = initial_path_aux->Tour[i];
	}

	new_path->cost = initial_path_aux->cost;
    new_path->bound = initial_path_aux->bound;
    new_path->length = initial_path_aux->length;
    new_path->node = initial_path_aux->node;
    new_path->isInTour = initial_path_aux->isInTour;
    return new_path;
}
