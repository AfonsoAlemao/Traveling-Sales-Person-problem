...
Solution *tsp_mpi(Inputs *input, int argc, char *argv[]) {
    ...
    int solved = 0, finish = 0, end = 1, procBestCost = rank;
    int counter_send = 0, flag_recF = 0, flag_rec9 = 0;
    int twice_density, auxProcBestCost = 0, flag_p0 = 0, dealer_p0 = 0;
    ...
    double BestTourCostAux = 0, BestTourCost, BestTourCostAuxx[2];
        ...
        #pragma omp parallel shared(BestTourCost, dealer) private(updateCost)
        {
            ...
            while (((int) queue[tid]->size != 0) && (flag != 1) && ((int) queue[tid]->size < dealer)) {
                ...
                if (numprocs > 1) {
                    ...
                    if (flag_rec2) {
                        /* If the received BestTourCost is better than the current one, update it */
