...
Solution *tsp_mpi(Inputs *input, int argc, char *argv[]) {
    ...
    int n_cities = get_n_cities(input), whistle = -1, global_tid = 0, exit_global = 0, twice = 0, dealer = 0, flag_rec2 = 0;
    ...
    while (((int) queue_p0->size != 0) && (flag_p0 != 1) && ((int) queue_p0->size < dealer_p0)) {
        ...
        work(queue_p0, n_cities, &BestTourCost, input, sol, current_path_p0, &flag_p0, rank, numprocs, BestTourCostAuxx, false, &procBestCost,&updateCost, auxProcBestCost, receiver, MYTAG);
    ...
    #pragma omp parallel shared(BestTourCost, dealer) private(updateCost)
    {
        ...
        while (((int) queue[tid]->size != 0) && (flag != 1) && ((int) queue[tid]->size < dealer)) {
            ...
            work(queue[tid], n_cities, &BestTourCost, input, sol, current_path, &flag, rank, numprocs, BestTourCostAuxx, true, &procBestCost, &updateCost, auxProcBestCost, receiver, MYTAG2);
        ...
        while (((int) queue[tid]->size != 0) && (flag != 1)) {
            ...
            current_path = queue_pop(queue[tid]);
            work(queue[tid], n_cities, &BestTourCost, input, sol, current_path, &flag, rank, numprocs, BestTourCostAuxx, true, &procBestCost, &updateCost, auxProcBestCost, receiver, MYTAG);
...
void work(priority_queue_t *queue, int n_cities, double *BestTourCost, Inputs* input, Solution *sol, Path* current_path, int *flag, int rank, int numprocs, double *BestTourCostAuxx, bool comm, 
         int *procBestCost, bool *updateCost, int auxProcBestCost, int receiver, int myTAG) {
